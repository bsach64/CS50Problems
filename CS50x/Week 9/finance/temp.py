...
def index():
    ...
    user_stocks = db.execute("SELECT * FROM owns WHERE user_id = ?", user_id)
    user_details = db.execute("SELECT cash FROM users WHERE id = ?", user_id)
    user_cash = float(user_details[0]["cash"])
    total = float(user_details[0]["cash"])
    for stock in user_stocks:
        current_info = lookup(stock["symbol"])
        price = float(current_info["price"])
        shares = int(stock["share_count"])
        stock["price"] = usd(price)
        stock["value"] = usd(price * shares)
        total += (shares * price)
    return render_template("index.html", user_stocks = user_stocks, cash = usd(user_cash), total = usd(total))
...
def buy():
    if request.method == "POST":
        ...
        if stock_data:
            ...
            balance = float(user_data[0]["cash"])
            spend = float(shares)*float(stock_data["price"])
            if spend > balance:
                return apology("Not enough money")
            else:
                balance = balance - spend
                db.execute("UPDATE users SET cash = ? WHERE id = ?", balance, user_id)
                db.execute("INSERT INTO transactions (user_id, price, share_count, transaction_type, transaction_date, symbol, stock_name) VALUES(?, ?, ?, 'bought', ?, ?, ?)", user_id, stock_data["price"], shares, str(datetime.now()), stock_data["symbol"], stock_data["name"])
                stocks = db.execute("SELECT symbol FROM owns WHERE user_id = ?", user_id)
                find = 0
                for stock in stocks:
                    if stock["symbol"] == stock_data["symbol"]:
                        ...
                        break
                if find == 1:
                    count = db.execute("SELECT share_count FROM owns WHERE symbol = ? AND user_id = ?", stock_data["symbol"], user_id)
                    share_count = int(count[0]["share_count"]) + int(shares)
                    db.execute("UPDATE owns SET share_count = ? WHERE symbol = ? AND user_id = ?", share_count, stock_data["symbol"], user_id)
                else:
                    db.execute("INSERT INTO owns (user_id, symbol, stock_name, share_count) VALUES(?, ?, ?, ?)", user_id, stock_data["symbol"], stock_data["name"], shares)
                return redirect("/")
        else:
            return apology("Invalid Stock Name")

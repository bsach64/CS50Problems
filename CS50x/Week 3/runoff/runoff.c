...
void tabulate(void)
{
    for (int v = 0; v < voter_count; v++)
    {
        ...
        {
            if (candidates[preferences[v][p]].eliminated == false)
            {
                candidates[preferences[v][p]].votes = candidates[preferences[v][p]].votes + 1;
                break;
            }
            else if (candidates[preferences[v][p]].eliminated == true)
            {
                p = p + 1;
            }
        }
...
{
    int minv = 100;
    for (int mv = 0; mv < candidate_count; mv++)
    {
        if (candidates[mv].eliminated == false)
        {
            if (candidates[mv].votes < minv)
            {
                minv = candidates[mv].votes;
            }
        }
    }
    return minv;
...
bool is_tie(int min)
{
    ...
    for (int t = 0; t < candidate_count; t++)
    {
        if (candidates[t].eliminated == false)
        {
            ...
            else if (candidates[t].votes != min)
            {
                x = x + 1;
            }
        }
